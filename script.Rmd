...
```{r FigS2,fig.height=8, fig.width=9}
...
ggsave("img/Fig.S2_biomas-plots-ModelDiag.png", width=9, height=9, units="in", dpi=300)
```
...
```{r FigS1, fig.height=8, fig.width=8, warning=FALSE}
### ENV Maps
...
ggsave("img/Fig.S1_biomas-plots-POC-NPP.png", width=8, height=8, units="in", dpi=300)
```

```{r FigS3,fig.height=4, fig.width=10, message=FALSE, warning=FALSE}
### Density Polts
## DENSITY NPP AND POC---------------------------------------------------
...
ggsave("img/Fig.S3_biomas-plots-ENV-DensityPlots.png", width=25, height=20, units="cm", dpi=300)
```
...
```{r FigS4,fig.height=4, fig.width=10, warning=FALSE}
...
ggsave("img/Fig.S4_biomas-plots-ProvId_Biomass.png", width=10, height=4, units="in", dpi=300)
...
ggsave("img/Fig.8_clusters_plot.png", width=5, height=5, units="in", dpi=300)
```
...
```{r FigS9,fig.height=5, fig.width=10, warning=FALSE}
library(maps)
library(metR)
...
medi.plt
ggsave("img/Fig.S9_biomas-plots-mediterenian.png", width=18, height=7, units="cm", dpi=300)
```
...
```{r FigS5,fig.height=4, fig.width=8}
# Bogorov, V.G., Vinogradov, M.D., Varonina, N.M., Kanaeva, I.P., and Suetova, I.A. 1968. Distribution of zooplankton biomass within the surficial layer of the world ocean. Dokl. Akad. Nauk. USSR, 182: 1205-1207
...
ggsave("img/Fig.S5_map-Bogrov.png", width=29.7, height=14, units="cm", dpi=300)
```
...
```{r FigS6}
# FAO. 1972. Atlas of the Living Resources of the Seas. Department of Fisheries, Food and Agriculture Organization of the United Nations, Rome.
nc <-   st_read(here::here("shp/Zookplankton_FAO-1972_WGS84/Zookplankton_FAO-1972_WGS84.shp")) |>
  mutate(bio_mg_m3=factor(Abundance, levels=c("<50","51-200","201-500",">500")))  |>
  st_transform(PROJ)

fao.map<- ggplot(nc, aes(fill = bio_mg_m3)) +
  theme_minimal() +
  geom_sf() +
  scale_fill_brewer(type = "qual") 
fao.map
ggsave("img/Fig.S6_map-FAO.png", width=29.7, height=14, units="cm", dpi=300)
```
```{r FigS7}
shp <- raster::shapefile("shp/Zookplankton_FAO-1972_WGS84/Zookplankton_FAO-1972_WGS84.shp")
shp <- spTransform(shp, CRS('+proj=longlat +datum=WGS84'))
## Set up a raster "template" for a 1 degree grid
ext <- extent(shp)
gridsize <- 1
...
df.fao<-cbind(coordinates(rr),as.data.frame(rr)) 

# put all the coordinates on a grid
closest_RANN <- RANN::nn2(query  = df.fao[, 1:2], 
                          data = grid[, 1:2],
                          k = 1)
fao.gridded <- cbind(df.fao, grid[closest_RANN$nn.idx,1:2 ]) |> 
  select(-x, -y) |> 
  mutate(bio=Abundance,
         Abundance_num=case_when(
    Abundance=="<50" ~25,
    Abundance=="51-200" ~(51+200)/2,
    Abundance=="201-500" ~(201+500)/2,
    Abundance==">500" ~500
  )) |> 
  mutate(bio=factor(bio,levels=c("<50","51-200","201-500",">500")))

right_join(diver_biom |> select(latitude, longitude, m2_poc),
                   fao.gridded |> select(latitude, longitude, bio),
                   by = c("longitude", "latitude")) |> 
  drop_na(m2_poc, bio) |> 
  select(longitude, latitude,total=bio,sum=m2_poc)   |> 
  plot.map2D(dim=4,  ylab = "meso", xlab = "epi",pal= "BlueYl")


ggsave("img/Fig.S7_map-FAOBio-2D.png", width=29.7, height=14, units="cm", dpi=300)
```
