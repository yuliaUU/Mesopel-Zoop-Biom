...
source("fun/theme_Publication.R")
```
```{r include=FALSE}
...
plt<- ggplot() +
  ...
  # final theme tweaks
  theme(legend.title = element_text(colour="black", size=14, face="bold",angle = 90), # adjust legend title
...
ggdraw() +
  ...
  draw_plot(legend, 0.7, .74, 0.4, 0.4)
...
plot_grid(m2_npp.plt,m2_poc.plt,
          ...
          labels = c("A","C","B","D"), label_size = 14, align = "h",hjust = -5, ncol= 2)
ggsave("img/Fig.1_biomas-plots-SLR.png", width=25, height=20, units="cm", dpi=600)
```

```{r FigS7+,fig.height=8, fig.width=9}
...
ggsave("img/Fig.S7_biomas-plots-ModelDiag.png", width=9, height=9, units="in", dpi=300)
```
...
```{r FigS5+, fig.height=8, fig.width=8, warning=FALSE}
### ENV Maps
...
ggsave("img/Fig.S5_biomas-plots-POC-NPP.png", width=8, height=8, units="in", dpi=300)
```

```{r FigS6+,fig.height=4, fig.width=10, message=FALSE, warning=FALSE}
### Density Polts
## DENSITY NPP AND POC---------------------------------------------------
...
ggsave("img/Fig.S6_biomas-plots-ENV-DensityPlots.png", width=20, height=8, units="cm", dpi=300)
```
...
map2_npp<- plot.map(npp.clim1[,c("longitude","latitude","m2")])+
  ...
  theme( plot.subtitle = element_text(size = title_size),
         text = element_text(size = text_size))
...
patch2_npp= map2_npp+theme(legend.position="none")+
  ldg2_npp+theme(legend.position="none")+plot_layout(widths=c(3,1.))
patch3_npp= map3_npp+theme(legend.position="none")+
  ldg3_npp+theme(legend.position="none")+plot_layout(widths=c(3,1.))
...
map2_poc<- plot.map(poc.clim[,c("longitude","latitude","m2")])+
  ...
  scale_color_gradientn(colours = my.colors,
       ...
       breaks=trans_breaks('log10', function(x) 10^x),
                      ...
                      barwidth=0.5,barheight=10,ticks=TRUE,
                          nbin = 10))+
  labs(subtitle="POC-based Variable Depth Model")+
  theme( plot.subtitle = element_text(size = title_size),
         text = element_text(size = text_size))
...
map3_poc<- plot.map(poc.clim[,c("longitude","latitude","m3")])+
  ...
  scale_color_gradientn(colours = my.colors,
    ...
    breaks=trans_breaks('log10', function(x) 10^x),
                      ...
                      barwidth=0.5,barheight=10,ticks=TRUE,
                          nbin = 10))+
  labs(subtitle="POC-based Classical Depth Model")+
  theme( plot.subtitle = element_text(size = title_size),
         text = element_text(size = text_size))
...
```{r Fig2+,fig.height=8, fig.width=18, message=FALSE, warning=FALSE}
...
(patch1 | patch2) + 
  ...
  theme(plot.tag = element_text("Times New Roman",face = "bold", size = 20,hjust = -3, vjust = 0))


ggsave("img/Fig.2_biomas-plots-climatology.png", width=17, height=8, units="in", dpi=300)
```
...
```{r FigS3+,fig.height=5, fig.width=12, warning=FALSE}
## 2D Biom vs SR + LDG
...
SRBio_2D + ldg.plt+  
  ...
  theme(plot.tag = element_text("Times New Roman",face = "bold", size = 16))

ggsave("img/Fig.S3_biomas-plots-Bio-SR-LDG.png", width=12, height=6, units="in", dpi=300)
```
...
```{r FigS4, fig.height=12, fig.width=10, message=FALSE, warning=FALSE}
title_size=16
text_size=16
# Define shared color scale limits and breaks
color_limits <- range(SR_Biom_avrg$SR, SR_Biom$SR, SR_Biom_tot$SR, na.rm = TRUE)
color_breaks <- c(0,10,30,50, 100, 200 ,300, 400, 500, 600)

# Biom vs Rarity
# Plot 1
p1 <- rbind(SR_Biom_avrg, SR_Biom %>% select(-tot_rar)) %>%
  mutate(
    basin = factor(basin, levels = c("Global", unique(SR_Biom$basin)))
  ) %>%
  arrange(avrg_rar) %>%
  ...
  scale_color_gradientn(
    ...
    breaks = color_breaks,
    guide = guide_colourbar(
      title.position = "top", barwidth = 0.5, barheight = 10, ticks = TRUE, nbin = 10
    )
  ) +
  labs(
    y = TeX(r'(Zoop C $(g \cdot m^{-2})$)'),
    color = "SR",
    x = "Average range-size rarity"
  ) +
  theme(
...
p2 <- SR_Biom_tot %>%
  mutate(
    basin = "Total range-size rarity (Global)",
...
# Combine plots with a shared legend and adjust layout
fig <- p2 / p1 +
  plot_layout(heights = c(1, 3), guides = "collect") +
  plot_annotation(tag_levels = "A") &
  theme(
    plot.tag = element_text("Times New Roman", face = "bold", size = 16, hjust = -3, vjust = 0))

ggsave("img/Fig.S4_biomas-plots-DiversityBiomass.png", plot=fig, width=10, height=15, units="in", dpi=300)

fig
```

# 7. Surface vs Deep biomass
...
#200-1000 m
p1 <- bogrov.gridded |> 
    select(longitude, latitude, bio_original) |> 
    plot.map(continious = FALSE, alpha = 1) +
    scale_color_manual(
        values = c("#313695", "#74add1", "#fee090", "#fdae61", "#f46d43", "#a50026"),
        guide = guide_legend(
            title = TeX(r'(\textbf{Biomass $(mg \cdot m^{-3})$})'),
            title.position = "left",
            override.aes = list(shape = 15, size = 6) # Set legend items to squares
        )
    ) +
    theme(
...
```{r Fig4+, fig.height=10, fig.width=16, message=FALSE, warning=FALSE}
### combine Stromberg & Bogrov maps
(patch_stromberg | bogorov) + 
  plot_layout(heights = 1) +  
  plot_annotation(tag_levels =  c('A', '1')) & 
  theme(plot.tag = element_text("Times New Roman",face = "bold", size = 16,hjust = -3, vjust = 0))

ggsave("img/Fig.4_biomas-plots-StrombergBogrov_200-1000.png", width=16, height=10, units="in", dpi=300)
```
